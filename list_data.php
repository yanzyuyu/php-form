...
<head>
    ...
    <title>Database Siswa - FormulirSiswa</title>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between">
        <div class="flex items-center gap-4 md:gap-10">
            ...
            <div class="hidden md:flex items-center gap-2">
                ...
                <a href="list_data.php" class="px-4 py-2 bg-indigo-600 text-white rounded-xl font-bold text-sm shadow-md shadow-indigo-200 transition-all">Database</a>
        ...
        <div class="flex items-center gap-2 md:gap-4">
            ...
            <a href="logout.php" class="hidden sm:flex p-2.5 bg-rose-50 text-rose-600 rounded-xl hover:bg-rose-600 hover:text-white transition-all shadow-sm border border-rose-100">
    ...
    <div id="mobileMenu" class="hidden md:hidden border-t border-slate-50 bg-white/95 backdrop-blur-xl animate-in slide-in-from-top duration-300">
        <div class="p-6 space-y-4">
            ...
            <div class="grid gap-2">
                ...
                <a href="list_data.php" class="flex items-center gap-3 px-4 py-3 bg-indigo-50 text-indigo-600 rounded-2xl font-bold transition-all">
...
<main class="max-w-7xl mx-auto px-4 sm:px-6 py-8 md:py-12">
    <div class="mb-8 md:mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
            <span class="text-indigo-600 font-black text-[10px] uppercase tracking-[0.2em] leading-tight">Data Repository</span>
            <h1 class="text-3xl md:text-4xl font-black text-slate-900 mt-2 tracking-tight leading-tight">Daftar Siswa</h1>
            <p class="text-slate-400 font-medium text-sm mt-1 leading-tight">Monitor seluruh data siswa yang diinput oleh staf.</p>
        </div>
    </div>
            ...
            <table class="w-full text-left min-w-[800px] md:min-w-full">
                <thead>
                    <tr class="bg-slate-50/50 border-b border-slate-50">
                        <th class="px-6 md:px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Siswa & Petugas</th>
                        <th class="px-6 md:px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Identitas</th>
                        <th class="px-6 md:px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Lahir</th>
                        <th class="px-6 md:px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Agama & JK</th>
                        <th class="px-6 md:px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Alamat</th>
                        <?php if (in_array($_SESSION['role'], ['owner', 'admin', 'petugas'])): ?>
                        <th class="px-6 md:px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Aksi</th>
                        <?php endif; ?>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50/30 transition-all group">
                        ...
                        <td class="px-6 md:px-8 py-6">
                            <span class="px-3 py-1.5 bg-slate-100 text-slate-600 text-[10px] font-black rounded-lg border border-slate-200 whitespace-nowrap">
                                NIS: <?= htmlspecialchars($row['nis']) ?>
                            </span>
                        </td>
                        <td class="px-6 md:px-8 py-6">
                            <p class="font-bold text-xs text-slate-800 whitespace-nowrap"><?= htmlspecialchars($row['tempat_lahir']) ?></p>
                            <p class="text-[10px] font-bold text-slate-400 uppercase mt-0.5 tracking-tighter whitespace-nowrap"><?= date("d M Y", strtotime($row['tanggal_lahir'])) ?></p>
                        </td>
                        <td class="px-6 md:px-8 py-6">
                            <div class="flex flex-col gap-1">
                                <span class="text-[10px] font-black text-indigo-600 uppercase whitespace-nowrap"><?= htmlspecialchars($row['agama']) ?></span>
                                <span class="text-[9px] font-bold text-slate-400 uppercase whitespace-nowrap"><?= $row['jenis_kelamin'] ?></span>
                            </div>
                        </td>
                        <td class="px-6 md:px-8 py-6 max-w-[180px]">
                            <p class="text-slate-500 text-[10px] font-medium leading-relaxed truncate group-hover:whitespace-normal transition-all duration-300"><?= htmlspecialchars($row['alamat']) ?></p>
                        </td>
                        <?php if (in_array($_SESSION['role'], ['owner', 'admin', 'petugas'])): ?>
                        <td class="px-6 md:px-8 py-6">
                            <div class="flex items-center justify-center gap-2">
                                <a href="edit_data.php?id=<?= $row['id'] ?>" class="p-2.5 bg-white border border-slate-100 text-indigo-600 hover:bg-indigo-600 hover:text-white rounded-xl transition-all shadow-sm">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                </a>
                                <button onclick="openDeleteModal('<?= $row['id'] ?>', '<?= addslashes($row['nama']) ?>')" class="p-2.5 bg-white border border-slate-100 text-rose-600 hover:bg-rose-600 hover:text-white rounded-xl transition-all shadow-sm">
...
<div id="deleteModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 modal-blur">
    <div class="bg-white w-full max-w-sm rounded-[2.5rem] p-8 md:p-10 shadow-2xl border border-slate-100 transform transition-all animate-in fade-in zoom-in duration-300">
        <div class="flex flex-col items-center text-center">
            ...
            <h3 class="text-xl md:text-2xl font-black text-slate-900 tracking-tight leading-tight">Hapus Data?</h3>
            <p class="text-slate-400 mt-2 font-medium text-sm">Anda akan menghapus data <span id="modalStudentName" class="text-slate-900 font-bold"></span>. Tindakan ini tidak bisa dibatalkan.</p>
            <div class="grid grid-cols-2 gap-4 w-full mt-8 md:mt-10">
                <button onclick="closeDeleteModal()" class="py-4 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-2xl font-black text-xs uppercase tracking-widest transition-all">Batal</button>
                <a id="confirmDeleteBtn" href="#" class="py-4 bg-rose-600 hover:bg-rose-700 text-white rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-lg shadow-rose-200 flex items-center justify-center">Hapus</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function openDeleteModal(id, name) {
        ...
        document.getElementById('confirmDeleteBtn').href = 'delete_data.php?id=' + id;

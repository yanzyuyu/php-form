...
<head>
    ...
    <title>Daftar - Formulir Siswa</title>
...
<body class="bg-[#fcfdfe] min-h-screen flex items-center justify-center p-4 md:p-6 text-slate-800">
    <div class="w-full max-w-md text-slate-800">
        <div class="flex flex-col items-center mb-8 md:mb-10 text-center">
            <h1 class="text-2xl md:text-3xl font-black tracking-tight">Daftar Akun</h1>
            <p class="text-slate-400 font-medium text-xs md:text-sm mt-1 leading-tight">Mulai digitalisasi data siswa hari ini</p>
        </div>
        ...
        <div class="bg-white p-8 md:p-10 rounded-3xl md:rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-indigo-50/50 relative overflow-hidden">
            <?php if (isset($error)): ?>
                <div class="animate-shake bg-rose-50 border border-rose-100 text-rose-600 p-4 rounded-2xl mb-8 flex items-center gap-4 shadow-sm text-slate-800">
                    ...
                    <div class="min-w-0">
                        <p class="text-[10px] font-black uppercase tracking-[0.2em] text-rose-400 leading-none">Gagal</p>
            ...
            <?php if (isset($success)): ?>
                <div class="bg-indigo-50 border border-indigo-100 text-indigo-600 p-4 rounded-2xl mb-8 flex items-center gap-4 shadow-sm text-slate-800">
                    <div class="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm text-indigo-500 shrink-0">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    </div>
                    <div class="min-w-0">
                        <p class="text-[10px] font-black uppercase tracking-[0.2em] leading-none text-indigo-400">Sukses</p>
                        <p class="text-xs font-bold mt-1 leading-tight text-slate-800 truncate"><?= $success ?> <a href="login.php" class="underline">Klik Masuk</a></p>
                    </div>
                </div>
            <?php endif; ?>
            ...
            <form method="POST" class="space-y-5 md:space-y-6 relative custom-form text-slate-800" novalidate>
                <div class="space-y-2 relative field-group text-slate-800">
                    ...
                    <input type="text" name="username" placeholder="Royyan" class="w-full px-5 py-4 bg-slate-50/50 border border-slate-100 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-600 focus:bg-white transition-all font-medium text-slate-800" required autofocus>
                </div>
                <div class="space-y-2 relative field-group text-slate-800">
                    <label class="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1 leading-tight text-slate-800">Email</label>
                    <input type="email" name="email" placeholder="nama@email.com" class="w-full px-5 py-4 bg-slate-50/50 border border-slate-100 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-600 focus:bg-white transition-all font-medium text-slate-800" required>
                </div>
                <div class="space-y-2 relative field-group text-slate-800">
                ...
                <div class="space-y-2 relative field-group text-slate-800">
                    <label class="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1 leading-tight text-slate-800">Konfirmasi Password</label>
                    <input type="password" name="confirm_password" placeholder="••••••••" class="w-full px-5 py-4 bg-slate-50/50 border border-slate-100 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-600 focus:bg-white transition-all font-medium text-slate-800" required>
                </div>
            ...
            <div class="mt-8 md:mt-10 pt-6 md:pt-8 border-t border-slate-50 text-center">
                <p class="text-sm text-slate-500 font-medium">Sudah punya akun? <a href="login.php" class="text-indigo-600 font-black hover:underline transition-all">Masuk Sekarang</a></p>
            </div>
        </div>
    </div>
